...
const DataProcessing_1 = __importDefault(require("../../utilities/DataProcessing"));
/**
    ...
    static modelTableDb(table) {
        ...
        if (table.foreignKeys.length > 0) {
            for (const foreignKey of table.foreignKeys) {
                const referenceName = foreignKey.referenced_table.charAt(0).toUpperCase() +
        ...
        for (const columnName in table.columns) {
            ...
            if (firstColumn) {
                ...
                text += `  ${columnName}: ${DataProcessing_1.default.getColumnType(column)};\n\n`;
                firstColumn = false;
            }
            else {
