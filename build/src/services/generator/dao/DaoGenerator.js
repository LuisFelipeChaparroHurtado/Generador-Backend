...
class DaoGenerator {
    ...
    static dao(table) {
        ...
        const lowercasedTableName = table.name.charAt(0).toLowerCase() + table.name.slice(1);
        ...
        for (const columnName in table.columns) {
            if (!isFirstColumn) {
                ...
                paramatercreate.push(`${lowercasedTableName}.${columnName} = parameter[${paramatercreate.length}];`);
                paramaterupdate.push(`${lowercasedTableName}.${columnName} = parameter[${paramatercreate.length}];`);
            }
            else {
